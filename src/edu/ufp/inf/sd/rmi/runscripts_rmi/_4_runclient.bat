...
@cd %ABSPATH2CLASSES%
@cls
java --module-path "C:\Users\danie\Desktop\Java\openjfx-11.0.2_windows-x64_bin-sdk\javafx-sdk-11.0.2\lib" --add-modules javafx.controls,javafx.fxml -cp %CLASSPATH% -Djava.security.policy=%CLIENT_SECURITY_POLICY% -Djava.rmi.server.codebase=%SERVER_CODEBASE% -D%JAVAPACKAGEROLE%.codebase=%CLIENT_CODEBASE% -D%JAVAPACKAGE%.servicename=%SERVICE_NAME_ON_REGISTRY% %JAVAPACKAGEROLE%.%CLIENT_CLASS_PREFIX%%CLIENT_CLASS_POSTFIX% %REGISTRY_HOST% %REGISTRY_PORT% %SERVICE_NAME_ON_REGISTRY%
